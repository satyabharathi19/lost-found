...
export default function ItemDetail({ item, onBack, currentUser }) {
  ...
  try {
    ...
    const response = await axios.delete(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/posts/${item._id}`);
  ...
  const handleFoundSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/responses`
, {
        postId: item._id,
