...
export default function MyListings({ user, viewType = 'owner' }) {
  ...
  const fetchResponses = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/responses/user/${user.id}`);
  ...
  const handleAcceptResponse = async (responseId) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/responses/${responseId}/accept`);
  ...
  const handleRejectResponse = async (responseId) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/responses/${responseId}/reject`);
      if (viewType === 'responder') {
        fetchMyResponses();
      } else {
        fetchResponses();
